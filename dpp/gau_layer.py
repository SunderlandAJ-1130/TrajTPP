...
###
# model = GAULayer(hidden_size=12)
# x = torch.rand(size=(256, 5, 12))
# pre_len = x.shape[1]
# masked_matrix = torch.tril(torch.ones(size=(x.shape[0], pre_len, pre_len), device=x.device), diagonal=0)
# o, a = model(x, output_attentions=True, attention_mask=masked_matrix)
...
class GatedAttentionUnit(nn.Module):
    ...
    def forward(
        ...
        if prior_info is not None:
            prior_info = prior_info.masked_fill(attention_mask==0, -1e9)
            z = torch.sigmoid(a+prior_info)
            a = z*a + (1-z)*prior_info
        ...
        # print(a[a!=-INF].min(), a.max(), A.min(), A.max())
        
        # 计算输出
        o = self.o_dense(u * torch.einsum("bmn,bnd->bmd", A, v))

        return (o, A) if output_attentions else (o,)
...
class GAULayer(nn.Module):
    ...
    def forward(
        ...
        # 投影变换
        gau_output = self.gau(
            hidden_states, attention_mask, sinusoidal_pos, output_attentions, prior_info=prior_info
        )
